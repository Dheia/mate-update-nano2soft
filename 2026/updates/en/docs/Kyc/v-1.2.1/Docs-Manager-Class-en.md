...
### 11. KYC Assessment Functions

#### `assessKycStatus`

Comprehensive assessment of KYC status for a specific owner (individual or company). The function checks all documents associated with the owner, identifies completed, missing, and expired documents, and calculates the completion percentage and overall verification score.
...
**Practical example (assessing a user with high risk):**
...
Assesses KYC status for a specific owner based on document `category` and a specific document type. This function does not rely on `risk_level` or `getMandatoryTypesForParty`; instead, it builds the list of required documents directly from the category and specified types.
...
| `$documentType` | `string\|array\|null` | Specific document type or array of types (optional). |
| `$options` | `array` | Additional options (`include_expired`, `include_pending`, `include_rejected`). |

**Practical example (assessing primary ID category for a user):**
