# API Documentation for Identity Verification (KYC) – Version 1.2.0
...
The `Nano3.Kyc` API provides secure and flexible endpoints for managing identity verification (KYC) documents in NanoSoft applications. Developers can use it to:

- Create, view, update, and delete documents.
- Upload identity files (passport, ID, signature) and attach them to documents.
...
1. **Use `temp_session_key` for unsaved models**: If you are building a multi-step interface, use a temporary session key to upload files first, then attach them after creating the document.
...
- [`DynamicAddIncludeKyc` Behaviors Documentation](./Docs-DynamicAddIncludeKyc-Behaviors-en.md)
