...
Listens to the `backend.form.extendFieldsBefore` event and adds a `partial` field to the specified tab, with support for `conditions`, `trigger`, and `depends_on`.
...
Adds a `kyc_documents_status` filter with status options to backend lists, with support for `conditions` and `depends_on`.
