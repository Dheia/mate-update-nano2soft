...
### تحديث شامل لإدارة إعدادات التحقق (OTP) في إضافة `VerifyCode`

**إضافة نظام إعدادات متكامل لإدارة خدمة التحقق من الرموز (OTP) عبر واجهة المستخدم الخلفية**

تم تنفيذ حزمة تطويرية تهدف إلى تمكين التحكم الكامل في إعدادات إضافة `Nano2\VerifyCode` من خلال واجهة إدارة NanoSoft App، بدلاً من الاعتماد على ملف `config.php` فقط. يتيح هذا التحديث للمطورين ومسؤولي النظام تعديل إعدادات OTP (مدة الصلاحية، قنوات الإرسال، وضع الاختبار، إعدادات الواجهة الأمامية ولوحة التحكم) بسهولة ومرونة، مع إمكانية تخصيص إعدادات مختلفة لكل نوع من عمليات التحقق (الجوال، تسجيل الدخول، البريد الإلكتروني) ولكل من الواجهة الأمامية ولوحة التحكم.
...
| المكون | الموقع | الوصف |
|--------|--------|-------|
| `Settings` | `Nano2\VerifyCode\Models\Settings` | نموذج الإعدادات الذي يرث `System\Behaviors\SettingsModel`، ويحتوي على جميع خيارات الإعدادات. |
| `fields.yaml` | `models/settings/fields.yaml` | تعريف حقول الإعدادات لعرضها في واجهة المستخدم الخلفية. |
| `Plugin` (تعديل) | `Plugin.php` | إضافة دالتَي `registerSettings` و `registerPermissions`، واستدعاء `Settings::registerConfig` في دالة `boot`. |
| `config.php` (تحديث) | `config/config.php` | إضافة كافة الإعدادات المطلوبة (بما في ذلك `url` و `param` لكل نوع) مع تعليقات توضيحية. |
| `Manager` (تعديل) | `Classes/Manager.php` | قراءة الإعدادات من `Config` التي تم دمجها مع قاعدة البيانات عبر `Settings::registerConfig`. |
| ملفات الترجمة | `lang/ar/lang.php` و `lang/en/lang.php` | إضافة مفاتيح الترجمة لجميع الحقول الجديدة في الإعدادات. |

---

### 2. تفاصيل التحديثات البرمجية

#### 2.1 إنشاء نموذج الإعدادات `Settings`
تم إنشاء نموذج يرث `System\Behaviors\SettingsModel`، مع تحديد `$settingsCode = 'nano2_verifycode_settings'` و `$settingsFields = 'fields.yaml'`. يحتوي النموذج على:

- دالة `initSettingsData()`: تقوم بتهيئة الإعدادات الافتراضية من ملف `config.php`.
- دالة `afterSave()`: إعادة تحميل الإعدادات في `Config` بعد حفظ أي تغيير.
- دوال مساعدة لبناء الهيكل المتداخل `buildStructuredSettings` و `deepMergeArr`.
- دالة ثابتة `registerConfig($isTraceLog, $force)` لدمج الإعدادات المخزنة في قاعدة البيانات مع إعدادات `config.php` وتحميلها في `Config`.

#### 2.2 إنشاء `fields.yaml`
تم تعريف جميع الحقول ضمن تبويبات منظمة:
- **عام**: الإعدادات العامة (مدة الصلاحية الافتراضية، قنوات الإرسال، رابط التفعيل العام، إلخ).
- **الاختبارات**: إعدادات وضع الاختبار (القيم الاختبارية، رمز الاختبار، تفعيل الاختبار للكل).
- **الواجهة الأمامية**: إعدادات منفصلة لكل من `mobile`، `check_login`، و `email` (مدة الصلاحية، الاختبار، رابط التفعيل، معامل الاستعلام).
- **لوحة التحكم**: نفس الهيكل للواجهة الخلفية.

تم استخدام أنواع الحقول المناسبة: `switch`، `number`، `text`، `taglist`، `dropdown`.

#### 2.3 تعديل ملف `Plugin.php`
أضيفت دالتان:
```php
...
```
كما تم استدعاء `\Nano2\VerifyCode\Models\Settings::registerConfig()` في دالة `boot()` لتطبيق الإعدادات عند بدء التشغيل.

#### 2.4 تحديث ملف `config.php`
أضيفت جميع الإعدادات المطلوبة مع تعليقات توضيحية مفصلة، بما في ذلك:
- الإعدادات العامة.
- إعدادات الاختبار.
- إعدادات الواجهة الأمامية (mobile, check_login, email) مع `url` و `param` لكل منها.
- إعدادات لوحة التحكم بنفس الهيكل.
- الصيغ البديلة للتوافق مع الدوال التي تبحث عن `frontend.expiration.mobile` إلخ.

#### 2.5 تحديث `Manager.php`
تم تعديل دوال قراءة الإعدادات مثل `getExpiration`, `getTestToken`, `getAllowTest` إلخ لقراءة القيم من `Config` التي أصبحت تحمل الإعدادات المدمجة. لم تتغير واجهة هذه الدوال، مما يضمن التوافق العكسي.

#### 2.6 إضافة ملفات الترجمة
تمت إضافة مفاتيح ترجمة لكل حقل جديد في `fields.yaml` ضمن المصفوفة `settings` في ملفات اللغة العربية والإنجليزية.

---

### 3. أمثلة تطبيقية

#### 3.1 الوصول إلى الإعدادات عبر الواجهة الخلفية
من قائمة الإعدادات في NanoSoft App، تظهر فئة جديدة باسم "إدارة التحقق من الرموز (OTP)"، وبداخلها رابط "إعدادات التحقق". عند النقر، تظهر واجهة تحتوي على جميع الإعدادات منظمة في تبويبات.

#### 3.2 تخصيص إعدادات مختلفة للواجهة الأمامية ولوحة التحكم
يمكن للمسؤول ضبط مدة صلاحية OTP للجوال في الواجهة الأمامية بـ 5 دقائق، وفي لوحة التحكم بـ 3 دقائق. كما يمكن تعيين رابط تفعيل مختلف للبريد الإلكتروني في الواجهة الأمامية (مثل `/verify-email`) وآخر في لوحة التحكم (مثل `/admin/verify-email`).

#### 3.3 تفعيل وضع الاختبار
يمكن تفعيل وضع الاختبار لتجربة النظام دون إرسال رسائل فعلية، وتحديد قائمة من الأرقام/البريد الاختبارية، وتحديد رمز OTP ثابت لهم (مثل `111111`).

#### 3.4 استخدام الإعدادات في `Manager`
جميع الدوال في `Manager` تستخدم `Config::get('nano2.verifycode::...')`، والتي تعيد الآن القيم المخزنة في قاعدة البيانات بعد الدمج مع ملف `config.php`. على سبيل المثال:
```php
$expiration = Config::get('nano2.verifycode::frontend.mobile.expiration', 5);
```
سيتم إرجاع القيمة المخزنة في قاعدة البيانات إذا وُجدت، وإلا القيمة من `config.php`.

---

### 4. القيمة المضافة

- **للمطورين ومسؤولي النظام**: أصبح من الممكن تعديل إعدادات OTP بسهولة عبر واجهة رسومية دون الحاجة لتعديل ملفات الكود أو استخدام متغيرات البيئة. هذا يقلل من الأخطاء ويسهل عملية الإعداد.
- **للمستخدمين النهائيين**: يمكن تخصيص تجربة التحقق (مدة الصلاحية، رابط التفعيل) لكل نوع من العمليات (تسجيل الدخول، استعادة كلمة المرور، تفعيل البريد) ولكل منصة (الواجهة الأمامية، لوحة التحكم).
- **للنظام**: تم إعادة هيكلة الإعدادات لتكون قابلة للتوسع بسهولة. إضافة أي إعداد جديد في المستقبل يتم عبر إضافته إلى `config.php` وإلى `fields.yaml` ونموذج `Settings`.
- **المرونة**: إمكانية تحديد إعدادات افتراضية في `config.php` مع إمكانية تغطيتها بقيم من قاعدة البيانات، مما يتيح التوافق مع البيئات المختلفة (التطوير، الإنتاج).

---

### 5. الخاتمة

يمثل هذا التحديث تحولاً مهماً في إدارة إضافة `VerifyCode` من الاعتماد على ملفات التهيئة الثابتة إلى نظام إعدادات ديناميكي عبر واجهة المستخدم الخلفية. تم تصميم الحل ليكون شاملاً، حيث يغطي جميع الإعدادات التي تستخدمها دوال `Manager`، مع الحفاظ على التوافق العكسي للمشاريع القائمة. أصبح بإمكان مسؤولي النظام التحكم بسهولة في سلوك خدمة OTP، مما يتيح تخصيصاً دقيقاً لتجربة التحقق في تطبيقات نانوسوفت.
