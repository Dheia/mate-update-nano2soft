...
**ملاحظة:** نفس الشيء ينطبق على `disable_auto_watermark` و `disable_upload`, `disable_delete`, `disable_get`,`disable_edit`.
...
#### 14.1 تحديث إعدادات حقل معين باستخدام `updateFieldConfig` (جديد في الإصدار 1.1.0)

**السيناريو:** بعد تسجيل النموذج، نريد تغيير الحد الأقصى لحجم حقل `image` من 1 ميجابايت إلى 2 ميجابايت، دون إعادة تسجيل النموذج بأكمله.
...
**التحقق:** استدعاء `getFieldConfig` سيعيد القيمة الجديدة.
...
**ملاحظات:**  
- الدالة تدمج الإعدادات الجديدة مع الحالية (`array_replace_recursive`) وتطبق التطبيع (`normalizeFieldDefinition`).  
- تمسح الكاش الخاص بهذا الحقل فقط، لذا لن تؤثر على حقول أخرى.  
- ترمي `ApplicationException` إذا كان النموذج أو الحقل غير مسجل.

#### 14.2 تحديث صلاحية `edit` لحقل معين (جديد في الإصدار 1.1.0)

**السيناريو:** إضافة صلاحية `edit` لحقل `image` للسماح باستبدال الصورة.
...
**التحقق:** يمكن استخدام `can` مع العملية `edit`:
...
## 17.1. التحقق من صلاحية `view` قبل عرض قائمة الملفات (مع مراعاة الإعدادات العامة)
...
## 17.2. استخدام الإعداد العام `disable_edit` (جديد في الإصدار 1.1.0)

**السيناريو:** تعطيل عملية استبدال الملفات على مستوى API بالكامل دون تعديل تعريفات الحقول.
...
if ($registry->isEditEnabledGlobally()) {
    echo "استبدال الملفات مفعل";
} else {
    echo "استبدال الملفات معطل عالمياً";
}
```

**ملاحظة:** تؤثر هذه القيمة على `FileUploadRegistry::can('edit')` وتمنع أي عملية `edit` حتى لو كانت الصلاحية موجودة.
