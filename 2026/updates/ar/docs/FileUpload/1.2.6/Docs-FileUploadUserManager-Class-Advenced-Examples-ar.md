...
> **ملاحظة الإصدار:** بدءاً من الإصدار 1.1.0، أصبحت عملية `edit` (استبدال الملفات) مدعومة وتستخدم نفس آلية التحقق من الصلاحيات مع تمرير `$operation = 'edit'`. تم تحديث الأمثلة لتشمل هذه العملية.
...
- التحقق من صلاحيات المستخدمين بناءً على نوعهم والصلاحيات المطلوبة (بما في ذلك صلاحية `edit` لاستبدال الملفات).
...
- التحقق من صلاحية `edit` لاستبدال الملفات.
...
- إضافة `Nano.FileUpload` مثبتة ومهيأة في تطبيقك (الإصدار 1.1.0 أو أحدث للاستفادة من صلاحية `edit`).
...
## 5. التحقق من صلاحيات مستخدم backend (عملية `add`)
...
## 6. التحقق من صلاحية `edit` لاستبدال ملف (جديد في الإصدار 1.1.0)

**السيناريو:** مستخدم backend يريد استبدال صورة منتج موجود (عملية `edit`). نتحقق من صلاحية `nano.shop.edit_image`.
...
if ($hasEditPermission) {
    ...
    echo "غير مسموح باستبدال الصورة";
}
```

**ملاحظات:**  
- صلاحية `edit` تُستخدم في `FileUploadService` عند اكتشاف عملية استبدال ملف في علاقة `attachOne`.
- يجب أن تكون الصلاحية مضبوطة في `FileUploadRegistry` على مستوى النموذج أو الحقل.

---

## 7. تعيين دالة مخصصة للتحقق من الصلاحيات (setPermissionChecker)
...
## 8. استخدام دالة `canUpload` في نموذج المستخدم (frontend)
...
## 9. دمج مع `FileUploadRegistry` للتحقق من صلاحية حقل معين (بما في ذلك `edit`)

**السيناريو:** قبل استبدال ملف لحقل معين، نستخدم `FileUploadRegistry::can` الذي يعتمد بدوره على `FileUploadUserManager`.
...
## 10. الحصول على نوع المستخدم (getUserType)
...
## 11. التعامل مع المستخدم غير المسجل (guest)
...
## 12. دمج مع `FileUploadService` في عملية رفع كاملة (بما في ذلك `edit`)
...
## 13. اختبار دالة `checkPermission` مع أنواع مختلفة من الصلاحيات
...
## 14. تخصيص دالة حل المستخدم بناءً على التوكن (JWT)
...
## 15. تخصيص دالة التحقق من الصلاحيات لتكون صارمة (مثل Laravel Gates)
...
## 16. معالجة أخطاء الصلاحيات في واجهة API

**السيناريو:** في متحكم API، نعيد استجابة مناسبة إذا كان المستخدم غير مصرح له (بما في ذلك عملية `edit`).
...
public function replaceImage(Request $request, $id)
{
    ...
    if (!$allowed) {
        return response()->json(['error' => 'لا تملك صلاحية استبدال الصورة'], 403);
    }

    // ... رفع الملف الجديد واستبداله
}
```
...
8. **لعمليات `edit` (استبدال الملفات)**، تأكد من أن المستخدم لديه الصلاحية المناسبة (`edit`) وأن `disable_edit` غير مفعّل في الإعدادات العامة.
...
## 19. دمج مع نظام الصلاحيات في نانوسوفت
...
- التحقق من صلاحيات المستخدمين حسب نوعهم (backend/frontend) بما في ذلك صلاحية `edit`.
