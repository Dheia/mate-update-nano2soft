...
**ملاحظات:**  
  > **ملاحظة:** يمكن تعطيل التحجيم التلقائي والعلامة المائية عالمياً عبر `disable_auto_resize` و `disable_auto_watermark` في الإعدادات العامة.
...
## 23. استبدال ملف موجود في علاقة `attachOne` (عملية `edit`) – جديد في الإصدار 1.1.0

**السيناريو:** منتج موجود لديه صورة رئيسية، نريد استبدالها بصورة جديدة. ستقوم الخدمة تلقائياً بحذف الصورة القديمة بعد نجاح رفع الجديدة، باستخدام معاملة قاعدة البيانات لضمان التكاملية.
...
if ($result['status']) {
    ...
    echo "معرف الملف القديم المحذوف: " . $result['process_data']['existing_file_deleted'];
} else {
    echo "فشل الاستبدال: " . $result['error'] . " (كود: " . $result['error_code'] . ")";
}
```

**مخرجات النجاح:** تحتوي `process_data` على `operation` و `existing_file_deleted`.

**ملاحظات:**  
- إذا فشل رفع الملف الجديد (مثلاً حجم كبير أو نوع غير مسموح)، لن يتم حذف الملف القديم بفضل المعاملة.  
- يجب أن يكون للمستخدم صلاحية `edit` على الحقل (أو النموذج) وأن لا يكون `disable_edit` مفعلاً عالمياً.

## 24. استخدام الأحداث الخاصة بعملية التعديل (`beforeEditDelete` و `afterEditDelete`) – جديد في الإصدار 1.1.0

**السيناريو:** تسجيل عملية استبدال الصورة في سجل النظام وإرسال إشعار.

```php
Event::listen('nano.fileupload.beforeEditDelete', function ($oldFile, $modelClass, $field, $model) {
    \Log::info("سيتم حذف الملف القديم {$oldFile->id} للموديل {$model->id}");
});

Event::listen('nano.fileupload.afterEditDelete', function ($oldFile, $modelClass, $field, $model) {
    \Log::info("تم حذف الملف القديم {$oldFile->id}");
    // إرسال إشعار للمسؤول
});

// ثم قم بعملية الرفع كالمعتاد
$result = $service->upload(Product::class, 'image', $newImage, ['model' => $product]);
```

## 25. ربط الملفات المؤقتة باستخدام `attachTempFiles` المحسّن (إرجاع استجابة موحدة) – جديد في الإصدار 1.1.0

**السيناريو:** رفع ملفات مؤقتة ثم ربطها بنموذج محفوظ، مع الحصول على تفاصيل العملية.
...
if ($attachResult['status']) {
    echo "تم ربط " . $attachResult['process_data']['attached_count'] . " ملفات بنجاح";
    foreach ($attachResult['data'] as $file) {
        echo "ملف: {$file['id']} - {$file['path']}\n";
    }
} else {
    echo "فشل الربط: " . $attachResult['error'];
}
```

**مخرجات `attachResult`:** هيكل موحد يحتوي على `code`, `status`, `message`, `data` (الملفات المرتبطة), `process_data` (عدد الملفات, الحقل, النموذج).

## 26. تجاوز التحقق من الصلاحية في الاختبارات باستخدام `skip_permission_check` – جديد في الإصدار 1.1.0

**السيناريو:** في بيئة الاختبار، نريد رفع ملفات دون الحاجة لتعيين صلاحيات حقيقية.

```php
$result = $service->upload(Product::class, 'image', $file, [
    'model' => $product,
    'skip_permission_check' => true, // يتجاوز أي تحقق من الصلاحية
]);
```

**ملاحظات:** يُستخدم هذا الخيار فقط في الاختبارات ولا ينصح باستخدامه في بيئة الإنتاج.

#### 12. استخدام المعاملة (`Db::transaction`) في عملية `edit` لضمان التكاملية

**السيناريو:** توضيح كيف تحافظ المعاملة على سلامة البيانات عند فشل رفع الملف الجديد.

```php
// هذا الكود داخلي في `upload`، لكن يمكن اختباره عن طريق محاكاة فشل (مثل رفع ملف بحجم كبير جداً)
$oldFileId = $product->image->id;
$result = $service->upload(Product::class, 'image', $tooLargeFile, ['model' => $product]);

if (!$result['status']) {
    // الملف القديم لم يتم حذفه
    $oldFileStillExists = File::find($oldFileId) !== null;
    echo "الملف القديم موجود: " . ($oldFileStillExists ? 'نعم' : 'لا'); // نعم
}
```

## 27. استخدام `updateFieldConfig` لتعديل قيود الحقل قبل الرفع (التكامل مع `FileUploadRegistry`)

**السيناريو:** تغيير الحد الأقصى لحجم الصورة مؤقتاً لرفع ملف كبير، ثم إعادته.
