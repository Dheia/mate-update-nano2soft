...
- نطاقات البحث والتصفية المتقدمة (Scopes).
- إدارة السجلات الافتراضية (`HasDefault`).
- خيارات القوائم المنسدلة للنماذج والفلاتر.
- دالة `getRecords` المتكاملة لجلب السجلات مع فلترة شاملة.
- التخزين المؤقت للكائنات والقوائم (`ListObjects`, `ListOptions`).
- علاقات متعددة الأشكال (Polymorphic) مع المالك (`owner`) والمحقق (`verifier`) والموضوع المرتبط (`subject`).
- **نظام حماية الوثائق المعتمدة** (Protect Verified Documents) مع صلاحيات مرنة.
- **آلية منع تكرار الوثائق** (Duplicate Prevention) للمالك نفسه.
- **معالجة تلقائية لعملية الاعتماد** (Auto-set verifier, verification score, status).
...
## نظام حماية الوثائق المعتمدة (Protect Verified Documents) – الجديد في 1.2.5

تم إضافة طبقة حماية متكاملة تمنع التعديل غير المصرح به على الوثائق المعتمدة (`is_verified = true`) وغير المنتهية. تتحكم في ذلك مجموعة من الإعدادات في `config.php` والدوال التالية:

### الإعدادات (`config.php`)

```php
'protect_verified' => [
    'is_check' => true,                     // تفعيل/تعطيل الحماية
    'allow_owner_modify_verified' => false, // السماح لمالك الوثيقة بتعديلها
    'backend_permissions' => 'nano3.kyc.documents.edit_verified', // صلاحية خاصة
    'allowed_fields' => [                   // الحقول المسموح تعديلها
        'verification_score',
        'is_verified',
        'status',
        'metadata',
...
'is_check_duplicate' => true, // منع تكرار نفس نوع الوثيقة لنفس المالك
```
...
| الدالة | النوع | الوصف |
| :--- | :--- | :--- |
| `canModifyDocument(?Document $doc, $user)` | `public static` | تحدد ما إذا كان المستخدم مسموحاً له بتعديل الوثيقة. تراعي الإعدادات والصلاحيات. |
| `shouldProtectVerifiedDocument(?Document $doc)` | `public static` | فحص سريع لمعرفة ما إذا كان يجب تطبيق الحماية على الوثيقة. |
...
1. عند محاولة حفظ وثيقة، تُستدعى `protectVerifiedDocument()` داخل `beforeSave`.
2. تتحقق الدالة من `canModifyDocument()` لمعرفة صلاحية المستخدم.
3. إذا كان التعديل مسموحاً، يُكمل الحفظ.
4. إذا كان ممنوعاً (Frontend أو مالك بدون صلاحية)، يتم منع التعديل بالكامل أو السماح فقط بالحقول المحددة في `allowed_fields`.
...
| `beforeSave` | قبل الحفظ. يتم فيها ضبط `companys_id` و `departments_id`، التحقق من التكرار، معالجة الاعتماد، وتطبيق الحماية. |
...
| `afterSave` | بعد الحفظ. تحديث الكاش، مسح كاش KYC، وإدارة السجل الافتراضي. |
| `afterDelete` | بعد الحذف. مسح كاش KYC. |
...
// اعتماد الوثيقة (يتم تعيين verified_at, verifier, verification_score تلقائياً)
$document->is_verified = true;
$document->save();

// التحقق من صلاحية التعديل
if (Document::canModifyDocument($document)) {
    $document->full_name = 'أحمد محمد علي';
    $document->save();
}
...
- `Nano\AuthApi\Classes\AuthHelpers` – للحصول على المستخدم الحالي.
